...
export const CreateView: React.FC<CreateViewProps> = ({ setOpenCreateModal }) => {
  ...
  const uploadMetadata = async (token: any): Promise<string | null> => {
    setIsLoading(true);
    const { name, symbol, description, image } = token;
    if (!name || !symbol || !description || !image) {
      notify({ type: "error", message: "Data is Missing" });
  ...
  const createTokenOrNFT = useCallback(async (token: any) => {
    setIsLoading(true);
    const lamports = await getMinimumBalanceForRentExemptMint(connection);
    const mintKeypair = Keypair.generate();
    ...
    try {
      ...
      const decimals = createType === "nft" ? 0 : Number(token.decimals);
      const amount = createType === "nft" ? 1 : Number(token.amount) * Math.pow(10, decimals);
      ...
      const tx = new Transaction().add(
        ...
        createMintToInstruction(mintKeypair.publicKey, tokenATA, publicKey!, amount),
        createMetadataInstruction
      );

      const signature = await sendTransaction(tx, connection, { signers: [mintKeypair] });
      setTokenMintAddress(mintKeypair.publicKey.toString());
      notify({ type: "success", message: `${createType.toUpperCase()} created successfully`, txid: signature });
  ...
  return (<div>/* Your JSX here as already written in previous message */</div>);
};
